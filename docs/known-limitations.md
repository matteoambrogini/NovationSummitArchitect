...
- Gli ultimi dieci wavetable sono sostituibili dall'utente e l'ordine numero/nome non
  dispone di una seconda verifica ufficiale sufficiente; restano `unverified`.
- I livelli A/B del Multi non hanno un range esplicito confermato; restano
  `unverified`.
...
- Reverb Size ha default 64 nella guida e nella tabella MIDI, ma 90 nella guida
  online; il parametro è `conflict`.
...
- L'assenza di un parametro dalla lista MIDI non prova che sia impossibile
  controllarlo: viene registrata come “non elencato”, non come protocollo inventato.
