...
- La generazione OpenAI richiede la shell Tauri; la build browser-only non possiede
  credenziali né un trasporto provider.
...
- Il keychain non è ancora collegato. In sviluppo la credenziale può essere letta da
  `.env.local`; nelle build distribuite deve arrivare dall'ambiente di processo.
